<!DOCTYPE html>
<html>
    
    <head>
       
        <Title> Suppression des matières : resultat </Title>
        <!--<link rel="stylesheet" type="text/css" href="http://localhost/e-Ecole-v3/assets/css/style.css"> -->
        
    </head>
    
    <body>
       
        <h3> Suppression des matières : resultat </h3>
        
        <p> La matière nommée <?php echo $mat_nom ?> a été supprimée de la base de données avec succès.</p> 
    
         
        <p> <a href='http://localhost/e-Ecole-v3/index.php/utilisateur_c/connecterAdministrateur'> Entrer au portail administrateur </a> </p>
  
    </body>
   
</html>